-- The child59 ledger is strictly append-only. Replace the runtime unlock function
-- so earliest-expiry allocation is decided before the ledger insert and no UPDATE
-- is ever issued against credit_ledger.
...
begin
  ...
  -- Allocate purchased credit from the earliest-expiring active lot. If no lot
  -- exists, the debit is covered by non-expiring grant/adjustment balance.
  ...
  if v_lot_id is not null then
    v_metadata := v_metadata || jsonb_build_object('purchase_lot_id', v_lot_id);
  end if;

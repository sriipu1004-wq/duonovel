-- Episode-level English translation cache and dedicated cost guard.
-- Additive only: existing story-generation tables/functions are intentionally untouched.
...
create table if not exists public.episode_translations (
  ...
  unique (episode_id, target_language, source_hash)
);

create index if not exists idx_episode_translations_episode_target_updated
  on public.episode_translations (episode_id, target_language, updated_at desc);

create table if not exists public.episode_translation_logs (
  id uuid primary key default gen_random_uuid(),
  request_id uuid not null unique,
  translation_id uuid references public.episode_translations(id) on delete set null,
  episode_id uuid not null references public.episodes(id) on delete cascade,
  source_hash text not null,
  target_language text not null,
  user_id uuid,
...
create index if not exists idx_episode_translation_logs_daily_counted
  on public.episode_translation_logs (created_at desc)
  where is_counted = true;

alter table public.episode_translations enable row level security;
alter table public.episode_translation_logs enable row level security;

revoke all on table public.episode_translations from anon, authenticated;
revoke all on table public.episode_translation_logs from anon, authenticated;
grant all on table public.episode_translations to service_role;
grant all on table public.episode_translation_logs to service_role;
...
begin
  ...
  select
    count(*)::bigint,
    coalesce(sum(coalesce(cost_estimate_jpy, 0)), 0::numeric)

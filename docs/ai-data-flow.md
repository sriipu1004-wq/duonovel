...
| Generated-story translation | OpenAI Responses API | `EPISODE_TRANSLATION_MODEL`; code default `gpt-5.4-mini` | `src/app/api/generated-story-translations/generate/route.ts` | Reader/user requests missing translation for generated story | Generated story text in translation batches and translation preferences | `generated_story_translations` (48-hour expiry in current route) | generated-story flow, separate from ordinary author works |
| AI Story generation | OpenAI Responses API | `OPENAI_TEXT_MODEL`; code default `gpt-5.4-mini` | `src/app/api/time-fit-stories/generate/route.ts` | User explicitly generates a story | User-selected generation conditions, optional custom request/tags and generation instructions | generated result returned to client; rate/cost logs contain request metadata but not the full prompt | AI-generation action/rate limits |
| AI Story continuation | OpenAI Responses API | `OPENAI_TEXT_MODEL`; code default `gpt-5.4-mini` | `src/app/api/time-fit-stories/continue/route.ts` | Owner explicitly asks to continue a saved AI-generated series | Latest AI-generated episode, continuity summary, titles and original generation context | new draft episode + continuity state + generation logs | owner-only and `isAiGeneratedSeries`; ordinary author works cannot enter this route |
...
The existing Privacy page described AI Story and Private Library processing but did not adequately describe AI processing of ordinary public author works. Child79 adds a focused public-work AI translation/word-explanation disclosure. Terms are not broadly rewritten in this child.

# LIB read 親チャット正史メモ 2026-04-19 d

更新日: 2026-04-19
配置: `docs/libread_master_plan_20260419_d.md`

## 今回の更新
- 子25の正式報告に基づき、`軽量化・快適化MVP` を完了扱いとして正式反映
- 子25では
  - 公開主要導線 `/` `/works/[seriesId]` `/read/[seriesId]/[episodeNumber]` の初段軽量化
  - `/search` の棚生成軽量化
  まで main 同期完了済みとして扱う
- 次に着手する本命を `広告掲載準備MVP` に更新
...
### 検索棚生成軽量化MVP
- `src/app/search/page.tsx` で `selectedPopularityMap` を `order === "popular"` の時だけ生成するよう変更

type AivisAudioQuery = Record<string, unknown> & {
  speedScale?: number;
  pitchScale?: number;
...
export function getAivisEngineBaseUrl(): string {
  const raw =
    process.env.AIVIS_ENGINE_URL?.trim() || "http://127.0.0.1:10101";

  return trimTrailingSlash(raw);
}

export async function synthesizeAivisWav({
  text,
  speaker,
  speedScale,
  pitchScale,
  ...
  prePhonemeLength,
  postPhonemeLength,
}: SynthesizeAivisWavInput): Promise<Uint8Array> {
  const normalizedText = text.trim();

  if (!normalizedText) {
    throw new Error("aivis_text_empty");
  }

  if (!Number.isInteger(speaker) || speaker < 0) {
    throw new Error("aivis_speaker_invalid");
  }

  const baseUrl = getAivisEngineBaseUrl();
  ...
  if (!audioQueryResponse.ok) {
    ...
    throw new Error(
      `aivis_audio_query_failed:${audioQueryResponse.status}:${message}`
    );
  }

  const audioQuery = (await audioQueryResponse.json()) as AivisAudioQuery;
  ...
  if (!synthesisResponse.ok) {
    ...
    throw new Error(
      `aivis_synthesis_failed:${synthesisResponse.status}:${message}`
    );
  }
  ...
  if (wavBytes.byteLength === 0) {
    throw new Error("aivis_audio_empty");
  }

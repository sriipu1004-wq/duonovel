...
export default function SavedWorkSearchClient({ order }: { order: "updated" | "added" }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: bookmarkData, error: bookmarkError } = await supabase.from("user_series_bookmarks").select("series_id, created_at").eq("user_id", user.id);
      if (bookmarkError) { if (active) { setError("ブックマーク一覧を取得できない。時間をおいて再度試す。"); setLoaded(true); } return; }
  ...
  if (!isLoggedIn) return <div className="mt-6 rounded-[28px] border border-dashed border-black/15 bg-neutral-50 p-6 text-sm leading-7 text-neutral-600">この一覧を表示するにはログインが必要。<Link href="/login?next=/search/saved" className="ml-2 underline underline-offset-4">ログインする</Link></div>;
  if (!sorted.length) return <div className="mt-6 rounded-[28px] border border-dashed border-black/15 bg-neutral-50 p-6 text-sm leading-7 text-neutral-600">{error || "まだブックマーク作品がない。"}</div>;

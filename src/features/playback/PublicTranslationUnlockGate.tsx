...
const COPY = {
  ...
  en: {
    ...
    loginAction: "Sign in",
    included: "Unlocking this episode with today's included translations.",
  ...
  ko: {
    ...
    loginAction: "로그인",
    included: "오늘 포함된 번역 이용 횟수로 이 화를 잠금 해제하고 있습니다.",
  ...
  return (
    <div className="mt-5 rounded-2xl border border-black/10 bg-white p-4 text-left">
      {entitlement.status === "login_required" ? (
        ...
        <p className="text-sm leading-6 text-neutral-700">{copy.included}</p>
      ) : (
        <>
      ...
      {confirmOpen ? (
        ...
        >
          <div className="w-full max-w-md rounded-[24px] bg-white p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-black">{copy.confirmTitle}</h2>
            <p className="mt-2 text-sm leading-6 text-neutral-600">{copy.confirmBody}</p>
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button
                ...
                disabled={busy}
                onClick={() => {
                  void Promise.resolve(onConfirmCredit()).finally(() => setConfirmOpen(false));
                }}
                className="rounded-full bg-black px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
              >

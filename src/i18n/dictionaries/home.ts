...
const ja: HomeDictionary = {
  ...
  ttsTitle: "長編翻訳の一貫性",
  ttsBody: "人名・固有名詞・組織名・世界観用語の作品単位用語集と、直前の公開話から必要範囲だけを使う限定コンテキストで訳語の揺れを抑える。作者は用語集の訳語を必要に応じて修正・固定できる。",
  aiTitle: "共有される公開翻訳",
  aiBody: "公開作品の翻訳は話・原文言語・翻訳言語・原文版ごとに保存し、同じ条件では再利用する。読者ごとに同じ翻訳を毎回作り直す構造ではない。",
...
const en: HomeDictionary = {
  ...
  ttsTitle: "Long-form translation consistency",
  ttsBody: "A work-level glossary for names, proper nouns, organizations, and world-specific terms is combined with limited context from the immediately preceding public episode to reduce translation drift. Authors can adjust and lock glossary translations when needed.",
  aiTitle: "Shared saved public translations",
  aiBody: "Public translations are saved by episode, source language, target language, and source version, then reused when those conditions match instead of regenerating the same translation for every reader.",
...
const ko: HomeDictionary = {
  ...
  ttsTitle: "장편 번역 일관성",
  ttsBody: "인명·고유명사·조직명·세계관 용어를 위한 작품 단위 용어집과 바로 이전 공개 회차의 필요한 범위만 쓰는 제한된 문맥으로 번역 흔들림을 줄입니다. 작가는 필요할 때 용어집 번역을 수정하고 고정할 수 있습니다.",
  aiTitle: "공유되는 저장 공개 번역",
  aiBody: "공개 작품 번역은 회차·원문 언어·번역 언어·원문 버전 기준으로 저장되고 같은 조건에서 재사용됩니다. 독자마다 같은 번역을 매번 다시 생성하는 구조가 아닙니다.",

...
function isShortStory(series: SeriesRow): boolean {
  const tags = Array.isArray(series.tags) ? series.tags.map(String) : typeof series.tags === "string" ? series.tags.split(/[\n,、]/u) : [];
...
async function fetchSeries(seriesId: string, supabase: Awaited<ReturnType<typeof requireOwnedSeries>>["supabase"]): Promise<SeriesRow | null> { const { data, error } = await supabase.from("series").select("*").eq("id", seriesId).single(); return error || !data ? null : data as SeriesRow; }
async function fetchEpisodes(seriesId: string, supabase: Awaited<ReturnType<typeof requireOwnedSeries>>["supabase"]): Promise<EpisodeRow[]> { const firstTry = await supabase.from("episodes").select("*").eq("series_id", seriesId); if (!firstTry.error) return (firstTry.data ?? []) as EpisodeRow[]; const secondTry = await supabase.from("episodes").select("*").eq("seriesId", seriesId); return secondTry.error ? [] : (secondTry.data ?? []) as EpisodeRow[]; }
...
export default async function WriteSeriesEditPage({ params }: PageProps) {
  ...
  return <div className={className}><WriteSeriesForm mode="edit" currentUserId={user.id} series={series} episodes={episodes} /></div>;
}

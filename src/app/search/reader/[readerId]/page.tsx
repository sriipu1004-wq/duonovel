...
export default async function ReaderSearchPage({ params, searchParams }: Props) {
  ...
  const [first, second, preference] = await Promise.all([
  ...
  for (const row of [
    ...
    if (
      ...
      row.is_public !== false &&
      row.public !== false &&
      !name.startsWith("Aivis ") &&
      !name.startsWith("VOICEVOX Nemo")
    ) {
  ...
  for (const episode of (episodesResult.data ?? []) as EpisodeRow[]) {
    ...
    if (!parentSeriesId || !series.has(parentSeriesId)) continue;
  ...
  for (const row of recordings.values()) {
    const id = text(row.series_id, row.seriesId) ?? "";
    const source = series.get(id);
    ...
    const item = works.get(id) ?? {
      id,
      title: text(source.title) ?? "無題",
      summary:
  ...
  const sorted = [...works.values()].sort((a, b) =>
    order === "popular"
      ? b.plays + b.likes * 10 - (a.plays + a.likes * 10)
  ...
  return (
    <main className="min-h-screen bg-white text-black">
      <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <p className="text-sm text-neutral-500">朗読作品一覧</p>
        <div className="mt-6 flex flex-wrap items-end justify-between gap-4 border-b border-black/10 pb-4">
          ...
          <div className="flex gap-2 text-sm">
            <Link
              href={`/search/reader/${encodeURIComponent(readerId)}?order=updated`}
              className={
                order === "updated"
            ...
            <Link
              href={`/search/reader/${encodeURIComponent(readerId)}?order=popular`}
              className={
                order === "popular"
                  ? "rounded-full bg-black px-4 py-2 text-white"
                  : "rounded-full border border-black/10 px-4 py-2"
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2">
            {sorted.map((work) => (
              <PublicWorkBoardCard
                ...
                authorName={`朗読 ${work.count}件 / いいね ${work.likes} / 再生 ${work.plays}`}
                latestPostedLabel={work.updatedAt}

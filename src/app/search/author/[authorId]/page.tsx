...
function tags(value: unknown): string[] { return Array.isArray(value) ? value.map(String).map((tag) => tag.trim()).filter(Boolean) : typeof value === "string" ? value.split(/[\n,、]/u).map((tag) => tag.trim()).filter(Boolean) : []; }

      ...
      <div className="mx-auto w-full max-w-7xl px-4 pb-20 pt-8 sm:px-6 lg:px-8">
        <section className="border-b border-black/10 pb-12">
          <div className="max-w-5xl">
            ...
            <p className="mt-5 max-w-4xl text-xl font-medium leading-9 text-neutral-800">
              {dictionary.lead}
            </p>
            <p className="mt-5 max-w-4xl text-sm leading-8 text-neutral-600 sm:text-base">
              {dictionary.description}
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={localizePath("/generate", locale)} className="rounded-full bg-black px-5 py-3 text-sm font-semibold text-white transition hover:bg-neutral-800">
                {dictionary.generate}
              </Link>
              <Link href={localizePath("/library", locale)} className="rounded-full border border-violet-200 bg-violet-50 px-5 py-3 text-sm font-semibold text-violet-900 transition hover:bg-violet-100">
                {dictionary.library}
              </Link>
              <Link href={localizePath("/search", locale)} className="rounded-full border border-black/10 bg-neutral-100 px-5 py-3 text-sm font-semibold text-neutral-800 transition hover:bg-neutral-200">
                {dictionary.explore}
              </Link>
              <Link href={localizePath("/register", locale)} className="rounded-full border border-sky-200 bg-sky-50 px-5 py-3 text-sm font-semibold text-sky-900 transition hover:bg-sky-100">
                {dictionary.signup}
              </Link>
            </div>
          </div>
        ...
        <section className="mt-12 rounded-[28px] border border-black/10 bg-neutral-50 p-6 sm:p-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              <Link href={localizePath("/search", locale)} className="rounded-full bg-black px-5 py-2.5 text-sm font-semibold text-white">
                {dictionary.explore}
              </Link>
              <Link href={localizePath("/guide", locale)} className="rounded-full border border-black/10 bg-white px-5 py-2.5 text-sm font-semibold text-neutral-800">
                {dictionary.guide}
              </Link>
              <Link href={localizePath("/subscription", locale)} className="rounded-full border border-black/10 bg-white px-5 py-2.5 text-sm font-semibold text-neutral-800">

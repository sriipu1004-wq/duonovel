...
async function resolveLikeCount(authorId: string): Promise<number> {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!existing.data) {
      const { error } = await adminSupabase.from("author_profile_likes").insert({
        author_id: authorId,
        user_id: user.id,
      });
    ...
    const likeCount = await resolveLikeCount(authorId);
